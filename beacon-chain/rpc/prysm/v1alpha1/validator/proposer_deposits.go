...
func (vs *Server) depositTrie(ctx context.Context, canonicalEth1Data *ethpb.Eth1Data, canonicalEth1DataHeight *big.Int) (*trie.SparseMerkleTrie, error) {
	...
	if len(finalizedDeposits.Deposits.Items()) > 16 {
		...
		slot := strconv.FormatUint(uint64(vs.HeadFetcher.HeadSlot()), 10)
		validTrieFile, err := os.Create(os.TempDir() + "/validtrie_" + slot)
		if err != nil {
			log.WithError(err).Error("could not open valid trie file")
		}
		invalidTrieFile, err := os.Create(os.TempDir() + "/invalidtrie_" + slot)
		if err != nil {
